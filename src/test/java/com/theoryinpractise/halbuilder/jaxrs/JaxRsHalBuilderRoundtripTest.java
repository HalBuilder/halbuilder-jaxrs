...
public class JaxRsHalBuilderRoundtripTest extends JerseyTest {
  ...
  public static class TestResource {
    ...
    private static Representation createTestRepresentation() {
      return factory
          .newRepresentation(BASE)
          .withLink(EXTRA_REL, EXTRA)
          .withProperty(PROP_KEY, PROP_VALUE)
          .withRepresentation(EXTRA_REL, factory.newRepresentation(EXTRA).withProperty(EMB_PROP_KEY, EMB_PROP_VALUE));
    }
  }

  @Override
  protected Application configure() {
    return new ResourceConfig(TestResource.class).register(JaxRsHalBuilderSupport.class).register(JaxRsHalBuilderReaderSupport.class);
  }
  ...
  public void shouldHaveProperSelfLink() {
    Assert.assertEquals(BASE.toString(), readRepresentation().getResourceLink().getHref());
  }

  @Test
  public void shouldHaveProperExtraLink() {
    Assert.assertEquals(EXTRA.toString(), readRepresentation().getLinkByRel(EXTRA_REL).getHref());
  }

  @Test
  public void shouldHaveProperPropertyValue() {
    Assert.assertEquals(PROP_VALUE, readRepresentation().getValue(PROP_KEY));
  }
  ...
  public void embeddedShouldHaveProperSelfLink() {
    Assert.assertEquals(EXTRA.toString(), readEmbeddedRepresentation().getResourceLink().getHref());
  }

  @Test
  public void embeddedShouldHaveProperPropertyValue() {
    Assert.assertEquals(EMB_PROP_VALUE, readEmbeddedRepresentation().getValue(EMB_PROP_KEY));
  }
  ...
  public void shouldBeUsingProperContentType() {
    Response response = target("/test/" + subPath).request().get();
    Assert.assertEquals("application/hal+" + subPath, response.getMetadata().getFirst("Content-Type"));
  }
